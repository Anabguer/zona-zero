/**
 * ZZ-012 HUMAN_GATE — tutorial contextual landscape + ghost/✓.
 * Requires: serve :8765
...

{
  ...
  await shot(page, '01-coach-need-food.png', '01 · Coach comida', 'Una pista · Núcleo · Construir pulsa');

  await page.evaluate(() => window.__zz.startBuild('farm'));
  await page.waitForTimeout(300);
  await shot(page, '02-coach-ghost-confirm.png', '02 · Ghost + tip ✓', 'Arrastrá fantasma · confirmá con ✓');
  ...
  await shot(page, '03-coach-staff-farm.png', '03 · Staff huerto', 'Avance natural tras construir');
  ...
  await shot(page, '04-coach-need-water.png', '04 · Coach agua', 'Siguiente tip · sin Continuar');
  ...
  await shot(page, '05-coach-ready.png', '05 · Listo avanzar', 'Tip final · pulso Avanzar día');

  await page.click('#zz-help', { force: true });
  await page.waitForTimeout(300);
  await shot(page, '06-help-filtered.png', '06 · Ayuda §21.3', 'Consulta · ghost/✓ · pan');

  await page.evaluate(() => {
    document.getElementById('zz-sheet').hidden = true;
  });
  await shot(page, '07-844x390.png', '07 · 844×390', 'Landscape tutorial');
  await ctx.close();
}

{
  ...
  const page = await ctx.newPage();
  await boot(page);
  await shot(page, '08-740x360.png', '08 · 740×360', 'Coach legible');
  await ctx.close();
}

{
  ...
  const page = await ctx.newPage();
  await boot(page);
  await shot(page, '09-desktop-coach.png', '09 · Desktop', 'Misma guía en mundo');
  await placeViaGhost(page, 'farm');
  await page.evaluate(() => {
    document.getElementById('zz-sheet').hidden = true;
  });
  await shot(page, '10-desktop-progress.png', '10 · Desktop progreso', 'Huerto + tip staffing');
  await ctx.close();
}
...
<title>ZZ-012 Review · Tutorial landscape (HUMAN_GATE)</title>
<style>
...
<header>
  <h1>ZZ-012 · Tutorial D1 contextual landscape (HUMAN_GATE)</h1>
  <p>Una pista · avance por acción · ghost+✓ · sin cascada Continuar · PENDIENTE DE REVISIÓN</p>
</header>
<div class="grid">
${gallery
  .map(
