/**

 * ZZ-012 HUMAN_GATE — tutorial contextual 2.8 (superficies + ghost/✓ + mundo).

 * Requires: serve :8765

 * node scripts/review-shots-zz012.mjs

 */

import { chromium } from 'playwright';

import { mkdirSync, rmSync, writeFileSync, readdirSync, copyFileSync } from 'fs';

import { join, dirname } from 'path';

import { fileURLToPath } from 'url';



const root = join(dirname(fileURLToPath(import.meta.url)), '..');

const out = join(root, 'docs', 'review');

const drive = 'G:\\Mi unidad\\Juegos\\Zona Zero\\Review';

const BASE = 'http://127.0.0.1:8765';

const HARNESS = `${BASE}/dev/harness-zz.html`;



mkdirSync(out, { recursive: true });

mkdirSync(drive, { recursive: true });

for (const dir of [out, drive]) {

  for (const f of readdirSync(dir)) {

    try {

      rmSync(join(dir, f), { force: true });

    } catch {

      /* ignore */

    }

  }

}



const gallery = [];

async function shot(page, file, title, note) {

  await page.waitForTimeout(420);

  await page.screenshot({ path: join(out, file), fullPage: false });

  gallery.push({ file, title, note });

}



async function boot(page) {

  await page.goto(`${HARNESS}#new=1&clear=1&name=Refugio%20Norte`, {

    waitUntil: 'networkidle',

    timeout: 60000,

  });

  await page.waitForFunction(() => window.__zzOk === true || window.__zzErr, { timeout: 60000 });

  const err = await page.evaluate(() => window.__zzErr || null);

  if (err) throw new Error(err);

  await page.waitForSelector('#zz-app:not([hidden])', { timeout: 15000 });

  await page.waitForTimeout(500);

  await page.evaluate(() => {

    const s = window.__zz.getState();

    s.resources.wood = Math.max(40, s.resources.wood || 0);

    s.resources.water = Math.max(30, s.resources.water || 0);

    if (s.population?.labor) s.population.labor.idle = Math.max(4, s.population.labor.idle || 0);

    s.flags.onboardingDone = false;

    s.flags.onboardingActive = true;

    s.flags.onboardingStep = 0;

    window.__zz.paint?.();

  });

}



async function placeViaGhost(page, type) {

  await page.evaluate((t) => {

    window.__zz.startBuild(t);

    const free = window.__zz.freeCells();

    const cell = free[1] || free[0];

    window.__zz.setGhost(cell[0], cell[1]);

    window.__zz.confirmBuild();

    document.getElementById('zz-sheet') && (document.getElementById('zz-sheet').hidden = true);

  }, type);

  await page.waitForTimeout(350);

}



const browser = await chromium.launch({ headless: true });



{

  const ctx = await browser.newContext({

    viewport: { width: 844, height: 390 },

    deviceScaleFactor: 2,

    isMobile: true,

    hasTouch: true,

    locale: 'es-ES',

  });

  const page = await ctx.newPage();

  await boot(page);



  await shot(page, '01-coach-need-food.png', '01 · Coach comida', 'Superficies · Núcleo · Construir');



  await page.evaluate(() => {

    window.__zz.panBy?.(22, -16);

    window.__zz.paint?.();

  });

  await shot(page, '02-pan-world.png', '02 · Pan mundo', 'Mundo > viewport · tip sigue');



  await page.evaluate(() => {

    window.__zz.recenter?.();

    window.__zz.paint?.();

  });

  await page.evaluate(() => window.__zz.startBuild('farm'));

  await page.waitForTimeout(350);

  await shot(page, '03-surfaces-ghost.png', '03 · Superficies + ghost', 'Áreas edificables · tip ✓/✕');



  await page.evaluate(() => {

    const free = window.__zz.freeCells();

    const cell = free[Math.min(3, free.length - 1)] || free[0];

    window.__zz.setGhost(cell[0], cell[1]);

    window.__zz.paint?.();

  });

  await shot(page, '04-ghost-moved.png', '04 · Ghost movido', 'Snap invisible · misma superficie');



  await placeViaGhost(page, 'farm');

  await page.evaluate(() => {

    document.getElementById('zz-sheet').hidden = true;

    window.__zz.paint?.();

  });

  await shot(page, '05-coach-staff-farm.png', '05 · Staff huerto', 'Avance natural tras construir');



  await page.evaluate(() => {

    const s = window.__zz.getState();

    const b = s.base.buildings.find((x) => x.type === 'farm');

    window.__zz.adjustWorkers(b.id, 1);

    document.getElementById('zz-sheet').hidden = true;

  });

  await shot(page, '06-coach-need-water.png', '06 · Coach agua', 'Otra colocación en superficies');



  await placeViaGhost(page, 'well');

  await page.evaluate(() => {

    const s = window.__zz.getState();

    const b = s.base.buildings.find((x) => x.type === 'well');

    window.__zz.adjustWorkers(b.id, 1);

    document.getElementById('zz-sheet').hidden = true;

  });

  await shot(page, '07-coach-ready.png', '07 · Listo + pan/zoom', 'Tip final · Avanzar día');



  await page.click('#zz-help', { force: true });

  await page.waitForTimeout(300);

  await shot(page, '08-help-filtered.png', '08 · Ayuda §21.3', 'Superficies · ghost · pan · recuperar');



  await page.evaluate(() => {

    document.getElementById('zz-sheet').hidden = true;

  });

  await shot(page, '09-844x390.png', '09 · 844×390', 'Landscape tutorial');

  await ctx.close();

}



{

  const ctx = await browser.newContext({

    viewport: { width: 740, height: 360 },

    deviceScaleFactor: 2,

    isMobile: true,

    hasTouch: true,

  });

  const page = await ctx.newPage();

  await boot(page);

  await shot(page, '10-740x360.png', '10 · 740×360', 'Coach legible');

  await ctx.close();

}



{

  const ctx = await browser.newContext({ viewport: { width: 1920, height: 1080 } });

  const page = await ctx.newPage();

  await boot(page);

  await shot(page, '11-desktop-coach.png', '11 · Desktop', 'Misma guía en mundo');

  await page.evaluate(() => window.__zz.startBuild('farm'));

  await page.waitForTimeout(350);

  await shot(page, '12-desktop-surfaces.png', '12 · Desktop construir', 'Superficies + ghost + tip');

  await ctx.close();

}



await browser.close();



writeFileSync(

  join(out, 'index.html'),

  `<!DOCTYPE html>

<html lang="es"><head>

<meta charset="utf-8"/>

<meta name="viewport" content="width=device-width, initial-scale=1"/>

<title>ZZ-012 Review · Tutorial 2.8 (HUMAN_GATE)</title>

<style>

body{margin:0;background:#12100c;color:#e8e0d0;font-family:system-ui,sans-serif}

header{padding:1.25rem 1.5rem;border-bottom:1px solid #333}

h1{margin:0;font-size:1.25rem}

p{margin:.35rem 0 0;opacity:.75;font-size:.9rem}

.grid{display:grid;gap:1rem;padding:1rem;grid-template-columns:repeat(auto-fill,minmax(320px,1fr))}

figure{margin:0;background:#1a1612;border:1px solid #333;border-radius:10px;overflow:hidden}

img{display:block;width:100%;height:auto;background:#0a0908}

figcaption{padding:.65rem .75rem;font-size:.85rem}

figcaption strong{display:block}

figcaption span{opacity:.7;font-size:.78rem}

</style></head><body>

<header>

  <h1>ZZ-012 · Tutorial D1 contextual (2.8 / HUMAN_GATE)</h1>

  <p>Superficies · ghost/✓ · mundo&gt;viewport · sin cascada Continuar · PENDIENTE DE REVISIÓN</p>

</header>

<div class="grid">

${gallery

  .map(

    (g) => `<figure><img src="${g.file}" alt="${g.title}"/><figcaption><strong>${g.title}</strong><span>${g.note || ''}</span></figcaption></figure>`

  )

  .join('\n')}

</div></body></html>`

);



{

  const b2 = await chromium.launch({ headless: true });

  const p = await b2.newPage({ viewport: { width: 1600, height: 2400 } });

  await p.goto(`file://${out.replace(/\\/g, '/')}/index.html`, { waitUntil: 'networkidle' });

  await p.waitForTimeout(900);

  await p.screenshot({

    path: join(out, 'review-contact-sheet.jpg'),

    type: 'jpeg',

    quality: 82,

    fullPage: true,

  });

  await b2.close();

}



for (const f of readdirSync(out)) {

  copyFileSync(join(out, f), join(drive, f));

}

console.log('ZZ-012 review OK', gallery.map((g) => g.file).join(', '));


