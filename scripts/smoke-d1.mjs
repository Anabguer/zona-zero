...
assert(state.layoutVersion === 1 && state.sectors?.recovered?.includes('core'), 'layout/sectors stub');
assert((state.mapCamera.zoom || 0) >= 2.2 && (state.mapCamera.zoom || 0) <= 2.5, 'zoom D1 mundo>viewport: ' + state.mapCamera.zoom);
...
assert(panDist >= 14 && panDist <= 26.01, 'pan lejos del HQ permitido D1: ' + panDist);
panCameraBy(state, 40, -40);
assert(Math.hypot(state.mapCamera.x - camp.x, state.mapCamera.y - camp.y) <= 26.01, 'pan clamp D1 ampliado');
recenterCamera(state);
assert(Math.abs(state.mapCamera.x - camp.x) < 0.01, 'recenter x');
assert((state.mapCamera.zoom || 0) >= 2.2, 'recenter zoom D1');
